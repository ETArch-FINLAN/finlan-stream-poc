...
public class Server {
	...
	public Server(String titleServer, String titleStream, String movie) {
		...
		if(!finSocket.register(titleServer)){
			System.err.println("Nao possivel se registrar como '"+titleServer+"'");
			finSocket.close();
			System.exit(1);
		}
		
		if(!finSocket.createWorkspace(titleStream)){
			System.err.println("Nao possivel criar o workspace '"+titleStream+"'");
		...
		if(!finSocket.joinWorkspace(titleStream)){
			...
			finSocket.deleteWorkspace(titleServer);
			finSocket.unregister();
			finSocket.close();
			System.exit(1);
		}
